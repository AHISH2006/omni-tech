...
const Omnitrix3D = ({ 
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/2 -left-12 -translate-y-1/2 w-16 h-8 bg-neutral-200 rounded-full border-2 border-neutral-400" style={{transform: 'translateZ(-10px)'}} />
      <div className="absolute top-1/2 -right-12 -translate-y-1/2 w-16 h-8 bg-neutral-200 rounded-full border-2 border-neutral-400" style={{transform: 'translateZ(-10px)'}} />
      <div className="absolute w-72 h-72 md:w-80 md:h-80 rounded-full bg-neutral-600 shadow-[0_60px_120px_rgba(0,0,0,0.9)] border-b-[20px] border-neutral-900" style={{transform: 'translateZ(-40px)'}} />
      ...
      <div className="relative w-72 h-72 md:w-80 md:h-80 rounded-full metallic-ring p-6 flex items-center justify-center border-4 border-neutral-500 overflow-hidden shadow-inner">
        ...
        <div onClick={(e) => { e.stopPropagation(); handleSelect(); }} className={`relative w-28 h-28 md:w-32 md:h-32 rounded-full border-[10px] transition-all duration-500 cursor-pointer flex items-center justify-center shadow-[0_0_40px_rgba(0,0,0,0.5)] ${isSelecting ? 'bg-red-600 border-red-300' : 'bg-neutral-900 border-neutral-700 hover:border-green-400'}`}>
        ...
        <button onClick={(e) => { e.stopPropagation(); rotate('prev'); }} className="absolute -left-12 md:-left-16 w-12 h-20 md:w-14 md:h-24 bg-neutral-700 border-x-4 border-neutral-800 rounded-xl shadow-2xl hover:bg-neutral-600 active:translate-x-2 transition-all flex items-center justify-center text-neutral-300"><ChevronLeft size={32} /></button>
        <button onClick={(e) => { e.stopPropagation(); rotate('next'); }} className="absolute -right-12 md:-right-16 w-12 h-20 md:w-14 md:h-24 bg-neutral-700 border-x-4 border-neutral-800 rounded-xl shadow-2xl hover:bg-neutral-600 active:-translate-x-2 transition-all flex items-center justify-center text-neutral-300"><ChevronRight size={32} /></button>
      </div>
  ...
  return (
    <div className="w-full h-screen bg-black flex flex-col md:flex-row items-center justify-center overflow-hidden p-4 md:p-8 relative">
      ...
      <div className={`w-full md:w-1/3 h-auto max-h-[35%] md:max-h-[70%] order-1 md:order-2 transition-all duration-700 transform ${showPanel ? 'translate-x-0 opacity-100' : 'translate-x-20 opacity-0'} z-50 p-2 md:p-0`}>
        <div className="bg-neutral-900/80 backdrop-blur-md border-2 border-green-500/30 rounded-2xl p-4 md:p-6 shadow-[0_0_30px_rgba(34,197,94,0.1)] relative overflow-hidden flex flex-col h-full">
          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-green-500/10 to-transparent h-20 w-full pointer-events-none" style={{ animation: 'scan 4s linear infinite' }} />
          <div className="flex items-center gap-3 mb-2 md:mb-4 border-b border-green-500/20 pb-2 md:pb-4">
            <Sparkles className="text-green-400" size={18} />
            <h3 className="text-green-400 font-black tracking-tighter uppercase text-sm md:text-base leading-none">Galactic Intel ✨</h3>
            <button onClick={() => setShowPanel(false)} className="ml-auto text-neutral-600 hover:text-red-500 transition-colors">✕</button>
          </div>
          <div className="flex-1 overflow-y-auto font-mono text-[10px] md:text-xs text-green-300/80 leading-relaxed scrollbar-hide">
            {isLoading ? <span className="text-green-700 animate-pulse">DECRYPTING DNA STREAM...</span> : <p>{aiResponse || "Select a module to begin decryption."}</p>}
          </div>
          <div className="mt-2 md:mt-4 pt-2 md:pt-4 border-t border-green-500/20 flex gap-2">
            <button onClick={() => callGemini(`How would a Ben 10 alien handle ${selectedItem.label}?`)} className="flex-1 px-2 py-1.5 bg-green-500/10 border border-green-500/30 rounded-md text-[8px] md:text-[10px] text-green-400 font-bold hover:bg-green-500/20 transition-colors"><Zap size={12} className="inline mr-1"/> TACTICAL</button>
            <button onClick={() => callGemini(`Scan lore for ${selectedItem.label}.`)} className="flex-1 px-2 py-1.5 bg-green-500/10 border border-green-500/30 rounded-md text-[8px] md:text-[10px] text-green-400 font-bold hover:bg-green-500/20 transition-colors"><Cpu size={12} className="inline mr-1"/> LORE</button>
          </div>
        </div>
      </div>
